...
const AWS_SNS = new aws_sdk_1.default.SNS(SNS_CONFIG);
...
const run = async () => {
    try {
        ...
        let hsr_rain = false;
        let hsr_could_rain = "";
        const hsr_weather_data = await axios_1.default.get(WU_BASE_URL + "/get_locality_weather_data", {
            params: {
        ...
        for (const l of localities) {
            const data = await axios_1.default.get(WU_BASE_URL + "/get_locality_weather_data", {
                params: {
                    locality_id: l.id,
                },
                headers: {
        ...
        for (const w of weather_data) {
            switch (w.id) {
                case localities[0].id:
                    if (!!w.weather.locality_weather_data.wind_direction &&
